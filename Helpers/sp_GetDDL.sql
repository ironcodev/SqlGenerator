...
--##############################################################################
             ...
             THEN @CrLf
                  ...
                  + CASE 
                      ...
                      ELSE
                        CASE
                     WHEN [index_columns_include] <> '---' 
                     THEN @CrLf + '   INCLUDE ('  COLLATE SQL_Latin1_General_CP1_CI_AS + [index_columns_include] + ')'   COLLATE SQL_Latin1_General_CP1_CI_AS
                  ...
                  + CASE 
                      WHEN [has_filter] = 1 
                      THEN @CrLf + '   WHERE '  COLLATE SQL_Latin1_General_CP1_CI_AS + [filter_definition]
                      ELSE ''
                    END
  ...
  IF @INDEXSQLS <> ''  COLLATE SQL_Latin1_General_CP1_CI_AS
    SET @INDEXSQLS = @CrLf + 'GO'  COLLATE SQL_Latin1_General_CP1_CI_AS + @CrLf + @INDEXSQLS;
--##############################################################################
--CHECK Constraints
--##############################################################################
...
--##############################################################################
  ...
  SELECT
    ...
    + ISNULL(
             @CrLf
             + 'if not exists(SELECT [name] FROM sys.objects WHERE TYPE=''R'' AND schema_id = ' COLLATE SQL_Latin1_General_CP1_CI_AS + CONVERT(VARCHAR(30),[OBJS].[schema_id]) + ' AND [name] = '''  COLLATE SQL_Latin1_General_CP1_CI_AS + quotename(OBJECT_NAME([COLS].[rule_object_id])) + ''')'  COLLATE SQL_Latin1_General_CP1_CI_AS + @CrLf
             + [MODS].[definition]  + @CrLf + 'GO' COLLATE SQL_Latin1_General_CP1_CI_AS +  @CrLf
             + 'EXEC sp_binderule  ' + quotename([OBJS].[name]) + ', ''' + quotename(OBJECT_NAME([COLS].[object_id])) + '.' + quotename([COLS].[name]) + ''''  COLLATE SQL_Latin1_General_CP1_CI_AS + @CrLf + 'GO'  COLLATE SQL_Latin1_General_CP1_CI_AS ,'')
  FROM [sys].[columns] [COLS] 
...
--##############################################################################
  ...
  SELECT
    @TRIGGERSTATEMENT = @TRIGGERSTATEMENT +  @CrLf + [MODS].[definition] + @CrLf + 'GO'
  ...
  IF @TRIGGERSTATEMENT <> ''  COLLATE SQL_Latin1_General_CP1_CI_AS
    SET @TRIGGERSTATEMENT = @CrLf + 'GO'  COLLATE SQL_Latin1_General_CP1_CI_AS + @CrLf + @TRIGGERSTATEMENT;
--##############################################################################
...
--##############################################################################
 ...
 --SELECT objtype, objname, name, value
  ...
  IF @EXTENDEDPROPERTIES <> '' COLLATE SQL_Latin1_General_CP1_CI_AS
    SET @EXTENDEDPROPERTIES = @CrLf + 'GO' COLLATE SQL_Latin1_General_CP1_CI_AS + @CrLf + @EXTENDEDPROPERTIES;
--##############################################################################
...
--note that this trims the trailing comma from the end of the statements
  ...
  SET @FINALSQL = @FINALSQL + ')' COLLATE SQL_Latin1_General_CP1_CI_AS + @CrLf ;
  ...
  PRINT @input
  SELECT @SCHEMANAME AS SchemaName,@TBLNAME AS TableName, @input AS [Item];
  RETURN 0;     
--##############################################################################
...
--##############################################################################
  ...
  SELECT
    ...
    + ISNULL(
             ...
             + ''')'  COLLATE SQL_Latin1_General_CP1_CI_AS
             + @CrLf
             + [MODS].[definition]  + @CrLf 
             + 'GO'  COLLATE SQL_Latin1_General_CP1_CI_AS +  @CrLf
